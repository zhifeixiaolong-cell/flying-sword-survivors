...
// 悬浮剑 (Stage 5 Commit A): 待命状态下展示在玩家身边的 M2 素剑.
//
// 浮游炮语义: 位置固定 (ANCHOR_DIR × DISTANCE), 朝向跟随光标.
// 剑形与实战飞剑共享 drawSword(), 通过 graphics.alpha 控制可见度.
// Commit A 不实现流动效果 (留给 Commit B 在新剑形上重做).
export class SwordHover {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    drawSword(this.graphics); // 一次性绘制, 不需要每帧重绘
    this.graphics.alpha = 0; // 起始隐藏
  }
  ...
  update(
    ...
    // 朝向: 跟随光标. 鼠标贴玩家时 (aimMag < 1e-6) 保持上一帧.
